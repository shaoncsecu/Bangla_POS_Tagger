...
"""
def read_preprocess_test_data(vocab, test_file):
    ...
    with open(test_file, 'r', encoding='utf-8') as data:
        ...
        for word in data:
            ...
            # sentence end marked by newline
            if not word.split():
    ...
    # Ensuring that we got all the words from the test corpus
    file_length = len(open(test_file, 'r', encoding='utf-8').readlines())
    assert(len(original) == file_length)
    assert(len(prep) == file_length)
...
def start():
    ...
    if not os.path.isfile(Config.VOCAB) and not os.path.isfile(Config.MODEL):
        ...
        print("Model File Already Exist...!!! Do you want to re-tain again?")
